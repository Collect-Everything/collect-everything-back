...
import { IBaseRepository, TListQuery } from "@ce/server-core";
import { ApiConfig } from "@ce/shared-core";
import _ from "lodash";

export abstract class SequelizeRepository<SequelizeModel extends Model>
  implements
    IBaseRepository<
      InferCreationAttributes<SequelizeModel>,
      InferAttributes<SequelizeModel>,
      SequelizeModel
    >
{
  id: string;
  ...
  create(data: InferCreationAttributes<SequelizeModel>) {
  ...
  async update(itemId: number, data: Partial<InferAttributes<SequelizeModel>>) {
  ...
  protected async executeUpdate(
    itemId: number,
    data: Partial<InferAttributes<SequelizeModel>>,
  ) {
